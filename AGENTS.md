...
│                             # Sidebar-Bar ist unter 1024px ausgeblendet
...
`DashboardConfig { version:1, settings?, bars?, nav?, header?, bottom?, views[] }` → `ViewConfig { id, title, icon, path?, layout, layoutOptions?, subview?, background?, showSidebar?, showHeader?, showBottom?, padding?, margin?, width?, sections[] }` → `SectionConfig { id, columnSpan?, cardOrientation?, padding?, margin?, cards[] }` (Abschnitte haben **kein** `title`/`icon` — Überschriften sind Cards vom Typ `section-title`) → `CardConfig { id, type, config, css?, size? }`.
...
- **Globale Bars**: Dashboard-Einstellungen → Tab „Bars“ wählt und konfiguriert je eine Card für Sidebar, Header und Bottom. Die mitgelieferten Default-Cards verwalten ihre inneren Slots über `BarCards.vue`. Die Sidebar bleibt ab 1024 px links stehen, hat rechts eine Trennlinie und ist auf Smartphones ausgeblendet; es erfolgt keine automatische Umordnung nach unten. Header und Bottom können pro View ein-/ausgeschaltet werden. Bottom ist standardmäßig sichtbar (`showBottom !== false`), Header standardmäßig ausgeblendet.
