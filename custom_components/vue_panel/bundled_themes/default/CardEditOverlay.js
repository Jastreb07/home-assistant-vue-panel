export default function ({ vue, useI18n, components, helpers }) {
  ...
  return {
    name: 'CardEditOverlay',
    components: {
      MdiIcon: components.MdiIcon,
    },
    props: {
      reserveResizeCorner: { type: Boolean, default: false },
    },
    emits: ['edit', 'duplicate', 'copy', 'cut', 'delete'],
    setup(props, { emit }) {
      void props
      const root = ref(null)
      ...
      async function toggleMenu() {
        ...
        await nextTick()
        positionMenu()
        menu.value?.querySelector('.vp-card-edit-action')?.focus()
      }
      ...
      function blockSurfaceScroll(event) {
