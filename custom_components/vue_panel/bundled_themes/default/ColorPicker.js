export default function ({ vue, components, helpers }) {
  ...
  const { defaultColor, formatColor, hsvToRgb, parseColor, rgbToHsv, toCss } = helpers
  ...
  return {
    ...
    setup(props, { emit }) {
       ...
       */
      const rgba = ref(parseColor(props.modelValue) ?? { ...defaultColor })
      ...
      function onAlpha(event) {
        commit({ ...rgba.value, a: Number(event.target.value) / 100 })
      }
      ...
      const eyeDropper = 'EyeDropper' in window ? new window.EyeDropper() : null
